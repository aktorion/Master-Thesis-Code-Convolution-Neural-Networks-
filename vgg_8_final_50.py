...
early_stopping = EarlyStopping(monitor='loss', patience=15)# stop after 15 worst results than best
...
class WeightsSaver(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        if self.epoch % self.N == 0:
            name = 'script1model8_50try01%05d.h5' % self.epoch
...
model = Sequential([
    ...
    Dense(300,kernel_initializer=initializers.RandomNormal(mean=0, stddev=0.01), bias_initializer='zeros'),
    Dense(1, activation='sigmoid', kernel_initializer=initializers.RandomNormal(mean=0, stddev=0.01), bias_initializer='zeros')
])
    
model.load_weights("script1model1_50try1700044.h5")

#We train only the last 1 convolutional layer
...
model.compile(optimizer=#Adam(lr=0.03, beta_1=0.94, beta_2=0.999, epsilon=None, decay=0.000001, amsgrad=False),
              #Adam(lr=0.005, beta_1=0.85, beta_2=0.97, epsilon=None, decay=0.00001, amsgrad=False),
              SGD(lr=0.0002, momentum=0.3, decay=0.000001, nesterov=True   ),
